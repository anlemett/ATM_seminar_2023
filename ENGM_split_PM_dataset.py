...
PM_south_df.reset_index(drop=False, inplace=True)

PM_south_df.to_csv(os.path.join(DATASET_DIR, output_filename1), sep=',', encoding='utf-8', float_format='%.3f', index = True, header = True)

PM_north_df.reset_index(drop=False, inplace=True)

PM_north_df.to_csv(os.path.join(DATASET_DIR, output_filename2), sep=',', encoding='utf-8', float_format='%.3f', index = True, header = True)
